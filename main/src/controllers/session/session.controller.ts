...
import { GetSessionsOfUserResponseSchema } from "./response.dto";
...
export class SessionController implements ISessionController {
  ...
  async getSessionsOfUser(req: Request, res: Response, next: NextFunction) {
    try {
      const payload = GetSessionsOfUserRequestSchema.parse({...req.query,limit:this.pageLimit});
      const userId = req.verifiedUser!.id;

      const result = await this.sessionService.getSessionsOfUser(payload,userId);
